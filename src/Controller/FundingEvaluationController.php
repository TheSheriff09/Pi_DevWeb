...
class FundingEvaluationController extends AbstractController
{
    ...
    public function evaluate(Request $request, EntityManagerInterface $em, int $id, ValidatorInterface $validator, MailerInterface $mailer): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            // --- Automated Whistleblower Logic ---
            $aiProbability = $this->getAiPrediction($application);
            if ($aiProbability !== null) {
                $shouldReport = false;
                $reason = "";

                if ($aiProbability < 0.40 && $decision === 'Accepted') {
                    $shouldReport = true;
                    $reason = "AI predicted a low success probability (" . round($aiProbability * 100) . "%), but evaluator ACCEPTED the application.";
                } elseif ($aiProbability > 0.70 && $decision === 'Rejected') {
                    $shouldReport = true;
                    $reason = "AI predicted a high success probability (" . round($aiProbability * 100) . "%), but evaluator REJECTED the application.";
                }

                if ($shouldReport) {
                    $evaluator = $em->getRepository(Users::class)->find($userId);
                    $evaluatorName = $evaluator ? $evaluator->getFullName() : "Unknown Evaluator";

                    $reclamation = new Reclamations();
                    $reclamation->setTitle("Suspicious Decision: App #" . $id);
                    $reclamation->setDescription("Automated Audit Report: " . $reason . " | Evaluator: " . $evaluatorName . " (ID: " . $userId . ")");
                    $reclamation->setStatus("Pending");
                    $reclamation->setCreatedAt(new \DateTime());
                    $reclamation->setRequestedId(0); // System ID
                    $reclamation->setTargetId($userId); // Reported Evaluator
    ...
    public function autoEvaluate(Request $request, EntityManagerInterface $em, int $id, MailerInterface $mailer): Response
    {
        ...
        $probability = $this->getAiPrediction($application);
        if ($probability === null) {
    ...
    private function getAiPrediction(Fundingapplication $application): ?float
    {
        ...
        return isset($output['probability']) ? (float)$output['probability'] : null;
    }
}
